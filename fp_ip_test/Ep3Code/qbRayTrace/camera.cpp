...
qbRT::Camera::Camera()
{
	...
	m_cameraPosition = qbVector<float>	{std::vector<float> {0.0, -10.0, 0.0}};
	m_cameraLookAt = qbVector<float>		{std::vector<float> {0.0, 0.0, 0.0}};
	m_cameraUp = qbVector<float>				{std::vector<float> {0.0, 0.0, 1.0}};
	m_cameraLength = 1.0;
	m_cameraHorzSize = 1.0;
	m_cameraAspectRatio = 1.0;
}

void qbRT::Camera::SetPosition(const qbVector<float> &newPosition)
{
	m_cameraPosition = newPosition;
}

void qbRT::Camera::SetLookAt(const qbVector<float> &newLookAt)
{
	m_cameraLookAt = newLookAt;
}

void qbRT::Camera::SetUp(const qbVector<float> &upVector)
{
	m_cameraUp = upVector;
}

void qbRT::Camera::SetLength(float newLength)
{
	m_cameraLength = newLength;
}

void qbRT::Camera::SetHorzSize(float newHorzSize)
{
	m_cameraHorzSize = newHorzSize;
}

void qbRT::Camera::SetAspect(float newAspect)
{
	m_cameraAspectRatio = newAspect;
}

// Method to return the position of the camera.
qbVector<float> qbRT::Camera::GetPosition()
{
	return m_cameraPosition;
}

// Method to return the LookAt of the camera.
qbVector<float> qbRT::Camera::GetLookAt()
{
	return m_cameraLookAt;
}

// Method to return the up vector of the camera.
qbVector<float> qbRT::Camera::GetUp()
{
	return m_cameraUp;
}

// Method to return the length of the camera.
float qbRT::Camera::GetLength()
{
	return m_cameraLength;
}

// Method to return the horizontal size.
float qbRT::Camera::GetHorzSize()
{
	return m_cameraHorzSize;
}

// Method to return the camera aspect ratio.
float qbRT::Camera::GetAspect()
{
	return m_cameraAspectRatio;
}

// Method to return the U vector.
qbVector<float> qbRT::Camera::GetU()
{
	return m_projectionScreenU;
}

// Method to return the V vector.
qbVector<float> qbRT::Camera::GetV()
{
	return m_projectionScreenV;
}

// Method to return the projection screen centre.
qbVector<float> qbRT::Camera::GetScreenCentre()
{
...
void qbRT::Camera::UpdateCameraGeometry()
{
	...
	m_projectionScreenU = qbVector<float>::cross(m_alignmentVector, m_cameraUp);
	m_projectionScreenU.Normalize();
	m_projectionScreenV = qbVector<float>::cross(m_projectionScreenU, m_alignmentVector);
...
bool qbRT::Camera::GenerateRay(float proScreenX, float proScreenY, qbRT::Ray &cameraRay)
{
	...
	qbVector<float> screenWorldPart1 = m_projectionScreenCentre + (m_projectionScreenU * proScreenX);
	qbVector<float> screenWorldCoordinate = screenWorldPart1 + (m_projectionScreenV * proScreenY);
